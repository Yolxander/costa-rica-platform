...
const AIRBNB_URL_REGEX = /^https?:\/\/(www\.)?airbnb\.[a-z.]+\/rooms\/\d+/i
const BOOKING_URL_REGEX = /^https?:\/\/(www\.)?booking\.com\/hotel\/[a-z]{2}\/[\w-]+\.[a-z]{2}(-[a-z]{2})?\.html/i

function isValidImportUrl(input: string): boolean {
    const trimmed = input.trim()
    if (!trimmed) return false
    try {
        const url = new URL(trimmed)
        const full = url.href
        return AIRBNB_URL_REGEX.test(full) || BOOKING_URL_REGEX.test(full)
    } catch {
        return false
    }
}
...
export default function ImportPage() {
    ...
    async function handlePreview(e: FormEvent) {
        ...
        if (!trimmedUrl || !isValidImportUrl(trimmedUrl)) {
        ...
        try {
            ...
            if (!res.ok || !json.success) {
                ...
                if (validationErrors) {
                    const firstError = (Object.values(validationErrors).flat()[0] as string) || "Invalid URL. Please check the listing URL."
                    setError(firstError)
                } else {
    ...
    return (
        <>
            <Head title="Import Listing - Sora" />
            <SidebarProvider
                style={
                    {
